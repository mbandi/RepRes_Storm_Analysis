...
```{r subset data} 
data <- subset(rawData, select = c(BGN_DATE, END_DATE, STATE, COUNTY, COUNTYNAME, EVTYPE, MAG, FATALITIES, INJURIES, PROPDMG, PROPDMGEXP, CROPDMG, CROPDMGEXP))
rawData <- NULL
```
...
## Create a new column with the correct event types, this duplicates some rows because there were multiple event types in the same EVTYPE entry
data <- data %>% pivot_longer(Astronomical.Low.Tide:Winter.Weather, names_to = "eventType", values_to = "isEvent") %>% subset(isEvent == TRUE)
...
data <- data %>% mutate(casualtiesMean = mean(casualtiesYear)/1e3, valueMean = mean(valueYear)/1e9, .by = eventType) ## year mean by type of event
ranking <- data %>% group_by(eventType) %>% filter(row_number()==1) %>% arrange(eventType) %>% select(eventType, casualtiesMean, valueMean) ## select one row for each group and sort by event type
```
...
casualtiesBar <- ggplot(data = head(arrange(ranking, desc(casualtiesMean)), 10)) + geom_bar(aes(x = eventType, y = casualtiesMean, fill = eventType), stat = "identity") + labs(x = "", y = "Casualties (thousands)") + theme(axis.text.x=element_blank())
